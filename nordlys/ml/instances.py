...
class Instances(object):
        ...
        with open(tsv_file, 'rb') as tsvfile:
           ...
           # Checks all the params are in the TSV file header
            ...
            for line in reader:
                ...
                # Generating instance
                if ins_id in self.instances:  # existing instance
    ...
    def group_by_property(self, property):
        ...
        for ins in self.get_all():
            property_dict[ins.get_property(property)].append(ins.id)
    ...
    def to_libsvm(self, file_name=None, qid_prop=None):
        ...
        # Getting features
        features = sorted(self.instances.values()[0].features.keys())
        ...
        for ins in sorted_instances:
            print ins.id
            out += ins.to_libsvm(features, qid_prop) + " # " + str(ins.id) + "\n"
            counter += 1
    ...
    def concatenate_inss(cls, inss_list):
        ...
        Args:
            inss_list: list of ml.Instances
        """
        instances = cls(None)
        ins_id = 0
        for inss in inss_list:
    ...
    def group_by_query(self):
        """
        Groups instances by queries.

        Returns:
             A dictionary {qid:[ins_id, ...], ...}
        """
        # return self.group_by_property("q_id")
        query_dict = defaultdict(list)
        for ins in self.get_all():
            query_dict[ins.q_id].append(ins)
        return query_dict
